...
function buildAdventureScreen() {
  ...
  // level nodes on the journey line
  const wrap = $('#adv-nodes');
  wrap.innerHTML = '';
  ADV_LEVELS.forEach((lvl, i) => {
    const unlocked = Adventure.unlocked(i);
    const node = document.createElement('button');
    node.className = 'adv-node' + (unlocked ? '' : ' locked') + (lvl.boss ? ' boss' : '');
    node.innerHTML = `
      <span class="adv-num">${lvl.boss ? '&#9760;' : i + 1}</span>
      <span class="adv-name">${lvl.name}</span>
      <span class="adv-state">${unlocked ? L('TAP TO PLAY') : L('LOCKED')}</span>`;
    if (unlocked) node.addEventListener('click', () => showStory(i));
...
function showLevelEnd(win) {
  const idx = game.adventure.level;
  const lvl = ADV_LEVELS[idx];
