...
class Fighter {
  ...
  tryFire(game, dt) {
    ...
    if (this.fireCooldown > 0 || this.ink < INK_COST) return;
    this.fireCooldown = FIRE_INTERVAL;
    this.ink -= INK_COST;
    this.firingVisual = 0.1;
    const spread = rand(Math.random, -0.075, 0.075);
    ...
    game.projectiles.push({
      ...
      team: this.team,
      owner: this,
      life: SHOT_RANGE / SHOT_SPEED * rand(Math.random, 0.75, 1.1),
    });
  }
  ...
  hurt(game, amount, attacker) {
    ...
    if (this.hp <= 0) {
      ...
      splat(this.x, this.y, 70, attacker.team);
  ...
  botUpdate(game, dt) {
    ...
    // aim: nearest visible enemy in range, otherwise paint ahead
    let foe = null, foeD = 300;
    ...
    if (foe) {
      this.aim = Math.atan2(foe.y - this.y, foe.x - this.x) + rand(Math.random, -0.12, 0.12);
    } else {
      this.aim = a;
    }
    ...
    if (this.botBurst < 1.0 || foe) this.tryFire(game, dt);

    // lob a bomb at clumps of enemies or big enemy turf
    if (this.bombs > 0 && foe && foeD > 120 && Math.random() < 0.01) {
